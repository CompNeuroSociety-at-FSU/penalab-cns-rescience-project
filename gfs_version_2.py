...
class gfs_object:
    ...
    def __init__(self):
        ...
        self.psi_neuron_morph.axon = Cylinder(length=90*um)
        self.psi_neuron_morph.dentrite = Cylinder(length=170*um)

        # Here is the morphology for the DLMn neuron, 2 diameters (one proximal one distal) and both one axon and dentrite
        # Nah, dude. The axon is tapered but the diamete for the neuron is not 
        self.dlmn_neuron_morph.proximal = Soma(diameter=2*um)
        self.dlmn_neuron_morph.distal = Soma(diameter=4*um)
        self.dlmn_neuron_morph.axon = Cylinder(length=50*um)
        self.dlmn_neuron_morph.dentrite = Cylinder(length=100*um)
